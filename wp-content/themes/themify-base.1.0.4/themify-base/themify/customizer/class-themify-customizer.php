			...
			if ( 'font' == $style || 'logo' == $style || 'tagline' == $style || 'decoration' == $style ) {
				...
				if ( isset( $font->family->name ) && '' != $font->family->name ) {
					if ( isset( $font->family->fonttype ) && 'google' == $font->family->fonttype ) {
						...
						wp_enqueue_style( 'custom-google-fonts' . md5($selector), themify_base_https_esc( 'http://fonts.googleapis.com/css' ) . '?family=' . str_replace( ' ', '+', $font->family->name ) . ':' . $font->family->variant . '&subset=' . implode(',', $user_subsets) );
					}
