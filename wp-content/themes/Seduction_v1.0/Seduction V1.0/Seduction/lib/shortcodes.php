...
function teamDutchImage($atts) {
	...
	$out = "<a href='$src' rel='lightbox_image'><img src='$theme/lib/imgthumb.php?src={$src}&h={$height}&w={$width}' title='$title' alt='$title' class='lightbox_thumb' /></a>";
...
function teamDutchGalleryImage($atts, $content = null) {
	...
	$out .= "<div width='100%'><div id='$ID'>$imgs</div></div>
...
function teamDutchLightboxLocalVideo($atts, $content = 'video') {
	...
	$out .= "
	<a rel='lightbox_video' href='#' onclick=\"jQuery.colorbox({href:'#$videoID', inline:true, title: '$title', width:$width + 40, height:$height + 70}); return false;\">$content</a>
...
function teamDutchVideo($atts, $content = 'video') {
	...
	if( in_array($type, array('.flv', '.f4v', '.mp4', '.mov')) ) {
		...
		$out .= "<a rel='lightbox_video' href='#' onclick=\"jQuery.colorbox({href:'$src', title: '$title', iframe:true, innerWidth:$width, innerHeight:$height}); return false;\">$content</a>";
		return $out;
	}
}
