...
function shortcode_check_list( $atts, $content = null ) {
    $content = str_replace(array('<ul>', '</ul>'), array('<ul class="checklist">','</ul><!-- st_checklist -->'), do_shortcode($content));
     return '<beforecheck>'.$content.'<aftercheck>';
}
...
function shortcode_tabs( $atts, $content = null)
{
  ...
  $toreturn .= '<div class="sc_tabs_header">'; 
        ...
        {
           $toreturn .= '<div title="'.($i-1).'" class="sc_tab">'.$atts['tab'.$i].'</div>'; 
        }
